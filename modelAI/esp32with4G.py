...
from mongodb_4G import MongoDBHandler
import time
...
# Tự set up hosts, replica set do 4G chặn
collection = MongoDBHandler(user="23127438", password="23127438", hosts=[
    "ac-82rops1-shard-00-00.azne9nl.mongodb.net:27017",
    "ac-82rops1-shard-00-01.azne9nl.mongodb.net:27017",
    "ac-82rops1-shard-00-02.azne9nl.mongodb.net:27017"
...
# ============================= Cấu hình Camera ======================
ESP32_URL = 'http://192.168.137.126/cam-hi.jpg'   # có thể là /capture hoặc /jpg tùy firmware
...
ID_GROUPS = {
    "1": {"biological"},
    "2": {"paper","cardboard","glass","metal","clothes"},
    "3": {"trash","battery","plastic", "shoes"},
}
...
pre_time_start = 0
last_sent_label = None  # chống gửi trùng nhãn

# =======================================================

def grab_frame(url, timeout=5):
    #print(f"Đang tải hình ảnh từ {url}...")
    resp = urllib.request.urlopen(url, timeout=timeout)
    if resp.status != 200:
        raise RuntimeError(f"Lỗi tải hình ảnh từ {url}: {resp.status}")
    data = resp.read()
    #print(data)
