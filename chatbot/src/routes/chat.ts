...
import { safeInput } from "../utils/guard";
import { classify } from "../nlu/classify";
import { TOOL_REGISTRY } from "../tools";
import { callEnhancedLLM, llmDirectAnswer } from "../llm/enhanced-ollama";
import { ToolCallSchema, ArgsSchemas } from "../llm/schemas";
...
(async () => {
  ...
  try {
    ...
    const r = await fetch(`${base}/api/tags`, { method: "GET" } as any);
    if (!r.ok) {
...
router.post("/chat", async (req, res) => {
  ...
  try {
    if (LLM_AVAILABLE) {
      ...
      const aa = ArgsSchemas[v.data.tool as keyof typeof ArgsSchemas].safeParse(v.data.args);
