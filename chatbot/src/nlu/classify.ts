...
import { mapBin } from "./aliases";

// Enhanced classification with semantic understanding
export function classify(raw: string): { tool: string, args: any } {
  ...
  const binSynonyms: Record<"plastic" | "organic" | "metal" | "paper", string[]> = {
  ...
  if (!detectedBin) {
    for (const [binType, synonyms] of Object.entries(binSynonyms)) {
      if (synonyms.some(syn => t.includes(syn))) {
        detectedBin = binType as "plastic" | "organic" | "metal" | "paper";
        break;
      }
    }
  }
