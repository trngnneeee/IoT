...
export async function getLatestReading(binId: string) {
  try {
...
export function computeFillPct(doc: any): number | null {
  if (!doc) return null;
...
export async function getStatusAll(bins: string[] = ["plastic","organic","metal"]) {
  try {
...
export async function getHistory(binId: string, hours = 24) {
  try {
    const col = (await getDB()).collection("bin_readings");
    const since = new Date(Date.now() - hours * 3600 * 1000);
...
export async function enqueueOpen(binId: string) {
  try {
...
function getBinStatus(data: any): string {
...
function getBinRecommendation(data: any): string {
...
function getOverallHealth(results: Array<{ bin: string, data: any }>): string {
...
function getTrendMessage(trend: string, changePercent: number): string {
  switch (trend) {
    case 'increasing':
