...
export async function generateAnswer(params: {
  ...
  tool: string;
  args: any;
  data: any;           // the raw tool result(s) after querying DB
}) {
...
`;
  ...
  try {
    ...
    if (!r.ok) throw new Error(`Ollama answer gen ${r.status}`);
 ...
 */
function generateFallbackResponse(userQuestion: string, tool: string, args: any, data: any): string {
  try {
      ...
      case "get_status_all":
        ...
        for (const [binId, binData] of Object.entries(data)) {
          if (binData && typeof binData === 'object' && 'exists' in binData && binData.exists) {
            const weight = (binData as any).weightKg !== null ? `${(binData as any).weightKg} kg` : "không có dữ liệu";
            const fill = (binData as any).fillPct !== null ? `${(binData as any).fillPct}%` : "không có dữ liệu";
